...
export class RotaService {
    ...
    private static readonly OPEN_CAGE_URL = 'https://api.opencagedata.com/geocode/v1/json?q=%s&key=%s&language=pt&format=json';
    private static readonly OSRM_URL = 'http://router.project-osrm.org/route/v1/driving/%.6f,%.6f;%.6f,%.6f?overview=false';
    ...
    private static readonly TARIFA_BASE = 5.00;
    private static readonly VALOR_KM = 1.50;
    private static readonly VALOR_MINUTO = 0.50;
    private static readonly SEGURO = 2.00;
    ...
    async calcularRota(viagem: Viagem): Promise<void> {
        try {
            RotaService.logger.log(`Iniciando cálculo da rota para viagem de '${viagem.partida}' para '${viagem.destino}'`);
            ...
            const distanciaEmKm = await this.calcularDistanciaRota(coordenadasOrigem, coordenadasDestino);
    ...
    private async getCoordenadas(endereco: string): Promise<Coordenadas> {
        try {
            ...
            const url = RotaService.OPEN_CAGE_URL.replace('%s', `${enderecoLimpo}, São Paulo - SP`).replace('%s', this.configService.get<string>('API_KEY'));
            ...
            const location = data.results?.find(resultado => 
                resultado.components?.city?.toLowerCase() === 'são paulo'
            );
            ...
            RotaService.logger.error(`Erro ao obter coordenadas para '${endereco}': ${error.message}`);
            throw new HttpException(
    ...
    private async calcularDistanciaRota(origem: Coordenadas, destino: Coordenadas): Promise<number> {
        try {
            await this.aplicarRateLimit();
            const url = RotaService.OSRM_URL.replace('%.6f,%.6f;%.6f,%.6f', 
    ...
    private calcularValorViagem(distanciaKm: number, tempoEstimado: number): number {
        return RotaService.TARIFA_BASE + 
